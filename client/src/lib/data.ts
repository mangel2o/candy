export const campuses = [
   "Álvaro Obregon",
   "Cancún",
   "Chihuahua",
   "Ciudad Juárez",
   "Ciudad Obregón",
   "Cuautitlan Izcalli",
   "Cuernavaca",
   "Culiacán",
   "Cumbres",
   "Durango",
   "Ferreria",
   "Guadalajara",
   "Guadalupe",
   "Hermosillo",
   "Laguna",
   "Las Américas",
   "Las Torres",
   "Los Mochis",
   "Mazatlán",
   "Merida",
   "Nuevo Laredo",
   "Puebla",
   "Queretáro",
   "Reforma",
   "Reynosa",
   "San Luis Potosí",
   "San Nicolás",
   "Tecmilenio Online",
   "Toluca",
   "Veracruz",
   "Villahermosa",
   "Zacatecas",
   "Zapopan",
   "Otro"
]

export const careersProfesional = [
   "Ingeniería en Desarrollo de Software",
   "Ingeniería en Mecatrónica",
   "Ingeniería en Sistemas de Computación Administrativa",
   "Ingeniería Industrial",
   "Ingeniería Industrial y de Sistemas",
   "Licenciatura en Administración de Empresas",
   "Licenciatura en Administración de Negocios",
   "Licenciatura en Administración Hotelera y Turística",
   "Licenciatura en Administración y Finanzas",
   "Licenciatura en Comercio Internacional",
   "Licenciatura en Creación y Desarrollo de Empresas",
   "Licenciatura en Diseño Gráfico y Animación",
   "Licenciatura en Gastronomía",
   "Licenciatura en Ingeniería en Computación Administrativa",
   "Licenciatura en Ingeniería en Desarrollo de Software",
   "Licenciatura en Ingeniería en Mecatrónica",
   "Licenciatura en Ingeniería en Sistemas de Computación Administrativa",
   "Licenciatura en Ingeniería Industrial",
   "Licenciatura en Ingeniería Industrial y de Sistemas",
   "Licenciatura en Mercadotecnia",
   "Licenciatura en Nutrición",
   "Licenciatura en Piscología",
   "Otra"
]

export const careersMaster = [
   "Maestría en Administración de Negocios Área Calidad y Productividad",
   "Maestría en Administración de Negocios Área Finanzas",
   "Maestría en Administración de Negocios Área Mercadotecnia",
   "Maestría en Administración de Negocios Área Recursos Humanos",
   "Maestría en Educación",
   "Maestría en Gestión de Tecnologías de Información",
   "Maestría en Liderazgo Positivo",
   "Otra"
]

export const careersEjecutive = [
   "Ingeniería Industrial y de Sistemas",
   "Ingeniería en Computación Administrativa",
   "Licenciatura en Comercio y Negocios Internacionales",
   "Licenciatura en Mercadotecnia y Publicidad",
   "Licenciatura en Administración",
   "Licenciatura en Derecho",
   "Licenciatura en Psicologia Empresarial",
   "Licenciatura en Educación y Desarrollo",
   "Otra"
]

export const careersSpecialty = [
   "Especialidad en Finanzas",
   "Especialidad en Innovación",
   "Especialidad en Recursos Humanos",
   "Otra"
]

export const levels = [
   "Bachillerato",
   "Profesional",
   "Ejecutivo",
   "Maestria",
   "Especialidad"

]
export const modalities = [
   "Semestre",
   "Tetramestre",
]

export const periods = [
   "Ene - May",
   "Ago - Dic"
]

export const years = [
   2016,
   2017,
   2018,
   2019,
   2020,
   2021,
   2022,
   2023,
   2024
]

export const statuses = [
   "Vacio",
   "Completo",
   "Pendiente",
   "Retenido",
   "Incompleto"
]