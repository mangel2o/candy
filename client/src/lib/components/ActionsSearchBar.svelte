<script>
	...
	$: actions.searchable = actions.filterable.filter((user) =>
		user[searchParameter].toString().toLowerCase().includes(searchQuery.toLowerCase())
	);
...
<ClickOutside fullWidth={true} on:clickoutside={() => (isFocused = false)}>
